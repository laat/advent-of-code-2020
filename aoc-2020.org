...
#+begin_src emacs-lisp
  (setq laat/make-adjacent-square-cache (make-hash-table :test 'equal))
    ...
    (cl-loop for i from (- row 1) to (+ row 1)
             append (cl-loop for j from (- col 1) to (+ col 1)
                             unless (or (and (eq row i) (eq col j))
                                        (> 0 i)
                                        (> 0 j)
                                        (<= (length (elt layout 0)) j))
                             collect (list i j))))

  (defun laat/count-taken (layout)
    (cl-loop for (i j) in (laat/cross (laat/range (length layout))
                                      (laat/range (length (cadr layout))))
             count (eq :taken (nth j (nth i layout)))))
  ...
  (defun laat/mutate-layout (layout)
    (let ((layout (copy-tree layout))
          (prev-layout (copy-tree layout))
          (squares (laat/cross (laat/range (length layout))
                               (laat/range (length (cadr layout))))))
      (cl-loop for (i j) in squares
               when (and (eq (nth j (nth i prev-layout)) :taken)
                         (<= 4 (laat/count-adjacent i j prev-layout)))
               do (setf (nth j (nth i layout)) :empty))

      (cl-loop for (i j) in squares
               when (and (eq (nth j (nth i prev-layout)) :empty)
                         (eq 0 (laat/count-adjacent i j prev-layout)))
               do (setf (nth j (nth i layout)) :taken))
      layout))
  ...
  (thread-last (laat/read-lines "data/11-example.txt")
...
The moment you realize that you have modelled it wrong, and have to restart for part 2
