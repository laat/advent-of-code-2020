    ...
    (cond ((eq (car lines) nil) (list north east))
          (t (seq-let (d n) (car lines)
               (let* ((rot (laat/deg-ting deg))
                      (dn (* (car rot) n));;)
                      (de (* (cadr rot) n))) ;;(* (cadr rot) n)))
                 (cond ((equal d "N") (laat/calc-ne (+ north n) east deg (cdr lines)))
                       ((equal d "S") (laat/calc-ne (- north n) east deg (cdr lines)))
                       ((equal d "E") (laat/calc-ne north (+ east n) deg (cdr lines)))
                       ((equal d "W") (laat/calc-ne north (- east n) deg (cdr lines)))
                       ((equal d "F") (laat/calc-ne (+ north dn) (+ east de) deg (cdr lines)))
                       ((equal d "R") (laat/calc-ne north east (- deg n) (cdr lines)))
                       ((equal d "L") (laat/calc-ne north east (+ deg n) (cdr lines)))))))))
...
#+begin_src emacs-lisp
  ...
  (defun laat/calc-ne-wp (n e x y lines)
    (cond ((eq (car lines) nil) (list n e))
          (t (seq-let (d v) (car lines)
               (let* ((rot (laat/rot-ting x y (cond ((equal d "R") (* -1 v))
                                                    ((equal d "L") v)
                                                    (t 0))))
                      (rx (car rot))
                      (ry (cadr rot))
                      (dx (* x v))
                      (dy (* y v)))
                 (cond ((equal d "N") (laat/calc-ne-wp n e (+ x v) y (cdr lines)))
                       ((equal d "S") (laat/calc-ne-wp n e (- x v) y (cdr lines)))
                       ((equal d "E") (laat/calc-ne-wp n e x (+ y v) (cdr lines)))
                       ((equal d "W") (laat/calc-ne-wp n e x (- y v) (cdr lines)))
                       ((equal d "F") (laat/calc-ne-wp (+ n dx) (+ e dy) x y (cdr lines)))
                       ((equal d "R") (laat/calc-ne-wp n e rx ry (cdr lines)))
                       ((equal d "L") (laat/calc-ne-wp n e rx ry (cdr lines)))))))))
