...
#+begin_src emacs-lisp :exports both
  (defun laat/make-masker (mask-str)
    (->> mask-str
         ...
         (seq-map-indexed
          (lambda (c i)
            (pcase c
              (`?1 `(lambda (m) (logior m ,(ash 1 i))))
              (`?0 `(lambda (m) (logand m ,(lognot (ash 1 i))))))))
         (-non-nil)
         (apply '-compose)))
  ...
  (list
   ...
   (funcall (laat/make-masker "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX") 2))
#+end_src
    ...
    (->> mask-str
         ...
         (seq-map-indexed
          (lambda (c i)
            (pcase c
              (?1 `(lambda (bs)
                     (-map (lambda (b) (logior b ,(ash 1 i))) bs)))
              (?X `(lambda (bs)
                     (append (-map (lambda (b) (logior b ,(ash 1 i))) bs)
                             (-map (lambda (b) (logand b ,(lognot (ash 1 i)))) bs)))))))
         (-non-nil)
         (apply '-compose)))
#+end_src
