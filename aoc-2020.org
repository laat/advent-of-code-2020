...
#+begin_src emacs-lisp
  (defun laat/count-n (row col layout)
    (cl-loop for i from 0 to (- row 1)
             unless (< i 0)
             unless (>= i (length layout))
             when (eq (elt (elt layout i) col) :taken)
             return 1
             finally return 0))
  (defun laat/count-s (row col layout)
    (cl-loop for i from (+ row 1) to (length layout)
             unless (< i 0)
             unless (>= i (length layout))
             when (eq (elt (elt layout i) col) :taken)
             return 1
             finally return 0))
  (defun laat/count-e (row col layout)
    (cl-loop for j from (+ col 1) to (length (elt layout 0))
             unless (< j 0)
             unless (>= j (length (elt layout 0)))
             when (eq (elt (elt layout row) col) :taken)
             return 1
             finally return 0))
  (defun laat/count-w (row col layout)
    (cl-loop for j from 0 to (- col 1)
             unless (< j 0)
             unless (>= j (length (elt layout 0)))
             when (eq (elt (elt layout row) j) :taken)
             return 1
             finally return 0))
  (defun laat/count-ne (row col layout)
    (cl-loop for i from 0 to (- col 1)
             for j = (+ row i)
             unless (< i 0)
             unless (< j 0)
             unless (>= i (length layout))
  ...
  (thread-last (laat/read-lines "data/11-example-empty.txt")
    (mapcar (lambda (line) (mapcar (lambda (c) (cond ((eq c ?L) :empty)
                                                     ((eq c ?#) :taken)))
                                   line)))
    (laat/count-n 3 3))
