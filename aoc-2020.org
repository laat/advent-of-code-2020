...
#+begin_src emacs-lisp
  ...
  (defun laat/string-to-chars (str)
    (mapcar 'identity str))
...
* COMMENT Day 18
#+begin_src emacs-lisp
  (defun laat/read-tokens (s)
    (pcase s
      (`nil nil)
      ((rx string-start (group (+ digit)) (* whitespace))
       (cons (string-to-number (match-string 1 s))
             (laat/read-token (substring s (match-end 0) ))))
      ((rx string-start "+" (* whitespace))
       (cons '+ (laat/read-token (substring s (match-end 0) ))))
      ((rx string-start "*" (* whitespace))
       (cons '* (laat/read-token (substring s (match-end 0) ))))
      ((rx string-start "(" (* whitespace))
       (cons "(" (laat/read-token (substring s (match-end 0) ))))
  ...
  (defun laat/match-open (tokens)
    (cl-loop with opens = 1
             for token in tokens
             for i from 0
             when (and (eq opens 1) (equal token ")"))
             return (list group (nthcdr (+ i 1) tokens))
             collect token into group
             do (if (equal token "(") (setf opens (+ opens 1)))
             do (if (equal token ")") (setf opens (- opens 1)))))

  (defun laat/parse (tokens)
    (pcase tokens
      (`(,a * . ,c) (list '* a (laat/parse c)))
      (`(,a + . ,c) (list '+ a (laat/parse c)))
      (`("(" . ,r)
       (let* ((m (laat/match-open r))
              (end (laat/parse (cadr m))))
         (if end (cons (laat/parse (car m)) end)
           (laat/parse (car m)))))
      (`(,x) x)))

  ;; (let ((tokens (laat/read-token (s-trim ex))))
  ;;   (format "%s" tokens)
  ;;   (format "%s" (laat/parse tokens)))
  ;; (laat/match-open '(1 2 3 ")" 4))
  ;; (laat/match-open '(1 "(" 2 3 ")" ")" 4))
  ;;(laat/read-token ex)
#+end_src
...
(format "%s" (laat/parse (laat/read-token example)))
#+end_src
...
#+begin_src emacs-lisp :var example=example
(eval (laat/parse (laat/read-token example)))
#+end_src
...
#+begin_src emacs-lisp :var ex=example

    (let ((tokens (laat/read-token (s-trim ex))))
      (format "%s" (cons (cadr tokens) (cons "(" (cons (car tokens) (cddr tokens)))))
      (format "%s" (cons (cadr tokens) (cons (car tokens) (cddr tokens))))
      (format "%s" (laat/parse (list 3 '* 4 '+ 3)))
      (format "%s" (laat/parse (list "(" 3 '+ 4 ")" '* 3)))
      (format "%s" (laat/parse tokens))
  )
...
#+begin_src emacs-lisp :var input=my-input
  (defmacro laat/times (n fn arg)
    (cons 'thread-last (cons arg (cl-loop for i below n collect fn))))

  (defun laat/add (as bs)
    (-zip-with '+ as bs))

  (defun laat/xcons (a b)
    (mapcan (lambda (x) (mapcar (lambda (y) (cons x y)) b)) a))

  (setq laat/neighbourhood
        (let ((dirs '(-1 0 1)))
          (laat/xcons dirs (laat/xcons dirs (laat/xcons dirs '(nil))))))

  (defun laat/neighbours (point)
    (--filter (not (equal point it))
              (--map (laat/add point it) laat/neighbourhood)))

  (defun laat/read-initial-alive (input)
    (->> input
         (s-trim)
         (s-lines)
         (-map 'laat/string-to-chars)
         (-map-indexed
          (lambda (x line) (--map-indexed (when (eq ?# it) (list x it-index 0)) line)))
         (apply 'append)))

  (defun laat/count-alive-neighbours (point points)
    (length (cl-intersection (laat/neighbours point) points
                             :test 'equal)))

  (defun laat/next-generation (points)
    (--> points
         (mapcan 'laat/neighbours it)
         (cl-remove-duplicates it :test 'equal)
         (-filter (lambda (candidate)
                    (let* ((is-alive (cl-find candidate points :test 'equal))
                           (count-neighbours (laat/count-alive-neighbours candidate points)))
                      (or (and is-alive (or (eq count-neighbours 2) (eq count-neighbours 3)))
                          (and (not is-alive) (eq count-neighbours 3)))))
                  it)))

  (length (laat/times 6 laat/next-generation (laat/read-initial-alive input)))
#+end_src
...
⚠️ Slow. 1 hour or more.
#+begin_src emacs-lisp :var input=my-input
  (setq laat/neighbourhood
        (let ((dirs '(-1 0 1)))
          (laat/xcons dirs (laat/xcons dirs (laat/xcons dirs (laat/xcons dirs '(nil)))))))
  ...
  (defun laat/read-initial-alive (input)
    (->> input
         ...
         (-map 'laat/string-to-chars)
         (-map-indexed
          (lambda (x line) (--map-indexed (when (eq ?# it) (list x it-index 0 0)) line)))
         (apply 'append)))
...
#+begin_src emacs-lisp :exports both
  (defun laat/parse-rule (line)
    (when (string-match
           (rx (group (+ digit)) "-" (group (+ digit)) " or "
               (group (+ digit)) "-" (group (+ digit)))
           line)
      (list (list (string-to-number (match-string 1 line))
                  (string-to-number (match-string 2 line)))
            (list (string-to-number (match-string 3 line))
                  (string-to-number (match-string 4 line))))))

  (defun laat/parse-rules (rules)
    (mapcan 'laat/parse-rule (split-string rules "\n")))

  (defun laat/check-rule (value rule)
    (and (<= (car rule) value) (<= value (cadr rule))))

  (defun laat/parse-tickets (tickets)
    (mapcan (lambda (line) (mapcar 'string-to-number (split-string line ",")))
            (cdr (split-string tickets "\n"))))

  (pcase-let*
      ((data (laat/read-file "data/16.txt"))
       (`(,rules-str _ ,nearby-str) (split-string data "\n\n"))
       (rules (laat/parse-rules rules-str))
       (nearby (laat/parse-tickets nearby-str)))
    (->> nearby
         (-remove (lambda (v) (-some? (lambda (r) (laat/check-rule v r)) rules)))
         (-sum)))
#+end_src
  ...
  (defun laat/parse-rule (line)
    (when (string-match
      ...
      (list
       ...
       `(lambda (value)
          (or
           (and (<= ,(string-to-number (match-string 2 line)) value)
                (<= value ,(string-to-number (match-string 3 line))))
           (and (<= ,(string-to-number (match-string 4 line)) value)
                (<= value ,(string-to-number (match-string 5 line)))))))))

  (defun laat/parse-tickets (tickets)
    (mapcar (lambda (line) (mapcar 'string-to-number (split-string line ",")))
            (cdr (split-string tickets "\n"))))

  (defun laat/valid-tickets (tickets rules)
    (-filter
     (lambda (ticket)
       (-all?
        (lambda (value)
          (-some?
           (lambda (rule) (funcall (plist-get rule :valid?) value))
           rules))
        ticket))
     tickets))

  (defun laat/transpose (list-of-lists)
    (-non-nil
     (cl-loop for list in list-of-lists
              for i from 0
              collect
              (cl-loop for list2 in list-of-lists
                       when (nth i list2)
                       collect (nth i list2)))))

  (defun laat/valid-rulenames (rules ticket-column)
    (->> rules
         (-map
          (lambda (rule)
            (when (-all? (plist-get rule :valid?) ticket-column)
              (plist-get rule :name))))
         (-non-nil)))

  (pcase-let*
      ((`(,rules-str ,my-str ,nearby-str) (split-string (laat/read-file "data/16.txt") "\n\n"))
       (rules (mapcar 'laat/parse-rule (split-string rules-str "\n")))
       (valid-rulenames (apply-partially 'laat/valid-rulenames rules))
       (my-ticket (car (laat/parse-tickets my-str)))
       (nearby (laat/valid-tickets (laat/parse-tickets nearby-str) rules))
       (tickets (cons my-ticket nearby))
       (column-candidates (mapcar valid-rulenames (laat/transpose tickets))))
    (cl-loop
     while (< 1 (apply 'max (mapcar 'length column-candidates)))
     for singles = (->> column-candidates
                        (-map (lambda (c) (when (= 1 (length c)) (car c))) )
                        (-non-nil))
     do
     (cl-loop for candidate in column-candidates
              for i from 0
              when (< 1 (length candidate))
              do (setf (nth i column-candidates)
                       (cl-set-difference (nth i column-candidates) singles)))
     finally return
     (cl-loop for candidate in column-candidates
              for i from 0
              when (s-starts-with? "departure" (car candidate))
              collect (nth i my-ticket) into ns
              finally return (apply '* ns))))
