...
#+begin_src emacs-lisp
  ...
  (defun laat/count-occupied-seats (seats)
    (cl-loop for v being the hash-values of seats
             count v))
  (defun laat/count-occupied-ajacents (seat seats ajacents)
    (cl-loop for ajacent in (gethash seat ajacents)
             collect ajacent))
  ...
  (let* ((lines (mapcar 'laat/string-to-char (laat/read-lines "data/11.txt")))
    ...
    (cl-loop
     ...
     finally return (laat/count-occupied-seats seats)))
